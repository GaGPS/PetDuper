...
if isUsingDelta() then
    ...
    deltaGui.Name = "Dupe+EggRandomizer+delta gui"
    ...
    stroke.Color = Color3.fromRGB(255, 203, 0)
    stroke.Thickness = 2
    ...
    -- Wrap pet label update in success/fail logic
    task.spawn(function()
        local originalText = label.Text
        label.Text = "🎲 Randomizing Egg..."
        wait(2)
        if math.random(1, 100) <= 90 then
            label.Text = originalText .. " 🟢 Success!"
        else
            label.Text = originalText .. " 🔴 Failed!"
        end
        wait(2)
    ...
    local fade = TweenService:Create(frame, TweenInfo.new(0.6), {BackgroundTransparency = 0})
    fade:Play()
    for _, child in pairs(frame:GetDescendants()) do
        if child:IsA("TextLabel") or child:IsA("TextButton") then
    ...
    btnStroke.Color = Color3.fromRGB(180, 140, 0)
    btnStroke.Thickness = 1
    btnStroke.Transparency = 0.3.CornerRadius = UDim.new(0, 8)
...
-- Run Pet Duper Logic (Compressed loadstring)
pcall(function()
